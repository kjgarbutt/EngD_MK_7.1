...
*/
public class SWiseAgent extends TrafficAgent implements Communicator, Serializable {
	...
	HashMap <SWiseAgent, Integer> intimateTies = new HashMap <SWiseAgent, Integer> ();
	HashMap <SWiseAgent, Integer> recentIntimateContact = new HashMap <SWiseAgent, Integer> ();
	 ...
	 */
	public SWiseAgent(String id, Coordinate position, Coordinate home, Coordinate work, Hotspots world){
	...
	void pickDefaultActivity(){
		...
		if(currentActivity == activity_travel && path != null){
			...
			world.schedule.scheduleOnce(this, 100 + world.random.nextInt(world.agentsLayer.size()));
			return;
		}
		...
		// if the Agent is traveling but has reached the end of its path, either transition into working or relaxing at home
			...
			if(geometry.getCoordinate().distance(work) <= Hotspots.resolution){
				...
				world.schedule.scheduleOnce(Math.max(time + 1, nextTime), 100 + world.random.nextInt(world.agentsLayer.size()), this);
			}
		...
		// default for no other case
			...
			world.schedule.scheduleOnce(time + 12, 100 + world.random.nextInt(world.agentsLayer.size()), this); // check again in an hour
		}
		...
		// If there IS a problem  //////////////////
			...
			// the Agent has enough information ////
				...
				if(currentActivity == activity_evacuate){
					...
					SWiseAgent contact = whoDoINeedToContact();
					if(contact != null){
	...
	void removeMe(){
		...
		for(Object o: this.knowledge.keySet()){
			...
			String sourceStr;
			if(source instanceof SWiseAgent)
				sourceStr = ((SWiseAgent)source).myID;
			else if(source == null)
				sourceStr = "null";
			else
	 ...
	 * @return
	 */
	SWiseAgent whoDoINeedToContact(){
		if(this.knowledge.size() > 0){
			for(SWiseAgent a: this.intimateTies.keySet()){
	...
	void intimateCommunicate(){
		...
		Iterator <SWiseAgent> iter = intimateTies.keySet().iterator();
	 ...
	 * @param a
	 */
	void intimateCommunicate(SWiseAgent a){
	 ...
	 */
	void exchangeAllInfo(SWiseAgent a, int time){
	 ...
	 * @param weight
	 */
	public void addContact(SWiseAgent contact, int weight){
	...
	public boolean equals(Object o){
		if(!(o instanceof SWiseAgent)) return false;
		else
			return ((SWiseAgent)o).myID.equals(myID);
	}
