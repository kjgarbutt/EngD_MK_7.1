...
public class AStar_SWise {
	 ...
	 *            - the goal node
	 * @param network
	 *            - the network in which the path should exist
	...
	public ArrayList<Edge> astarPath(GeoNode start, GeoNode goal, Network network) {
		...
		// System.out.println("A* Problem: graph has only " + closedSet.size() + " nodes
		// associated with it");
		return null;
	}

	/**
	 * Finds a path between the start and a set of goal nodes, within the given
	 * network
	 * 
	 * @param start
	 *            - the node from which to start
	...
	public ArrayList<Edge> astarPath(GeoNode start, ArrayList<GeoNode> goal, Network network) {
		...
		while (openSet.size() > 0) { // while there are reachable nodes to investigate
			...
			AStarNodeWrapper x = findMin(openSet); // find the shortest path so far
			if (goal.contains(x.node)) { // we have found the shortest possible path to the goal!
											// Reconstruct the path and send it back.
	 ...
	 * @return an ArrayList of Edges that lead from the given Node to the Node from
	...
	/**
	 * Measure of the estimated distance between two Nodes. Takes into account
	 * whether either of the GeoNodes entails a delay
	 * 
	 * @param x
	 * @param y
